...
def run_eda(target_day="Tuesday-20-02-2018"):
    print(f"Starting EDA for {target_day}...")
    ...
    # We want ~50,000 benign and ~50,000 attack flows to balance the view
    benign_dfs = []
    attack_dfs = []

    for f in sorted(files, key=os.path.getsize, reverse=True): # Start with largest files which likely have attacks
        ...
        if len(benign) > 0:
            # Randomly sample benign traffic so we don't load 6 million rows into memory
            benign_dfs.append(benign.sample(min(len(benign), 5000), random_state=42))

        # Break early if we have enough sample data for clear visualizations
    ...
    # Stratified sampling for final combined df
    ...
    print(f"Loaded balanced subset: {len(combined)} total samples ({len(sampled_benign)} Benign, {len(sampled_attack)} Attack)")
    ...
    print("Generating Feature Distribution Boxplot...")
    plt.figure(figsize=(10, 6))
    sns.boxplot(x=label_col, y='frame.len_mean', data=combined, palette='Set2')
    plt.yscale('log')
    plt.title("Average Frame Length Distribution (Benign vs DDoS)")
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "frame_len_dist.png"), dpi=300)
    plt.close()
